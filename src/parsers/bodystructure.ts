...
export function parseBodyStructure(data: string): ImapBodyStructure {
  ...
  //     disposition: { type: "ATTACHMENT", parameters: {} }
  //   };
  // }
}
...
//   ) {
//     bodyStructure.disposition.type = dispositionMatch[1].toUpperCase();
...
//             }
//           }
//           bodyStructure.disposition.parameters = dispParams;
//         } catch (error) {
...
//           bodyStructure.disposition.type = parseStringValue(dispositionParts[0])?.toUpperCase() || "ATTACHMENT";

//           if (dispositionParts.length >= 2) {
//             bodyStructure.disposition.parameters = parseParameterList(dispositionParts[1]);
//           }
//         }
//       } catch (error) {
...
function isAttachment(part: ImapBodyStructure): boolean {
  ...
  if (part.disposition.type === 'ATTACHMENT') {
    return true;
  }
  ...
  if (part.type === 'MESSAGE' && part.subtype === 'RFC822' && !part.disposition.type) {
    return true;
  }
...
export function findAttachments(
  ...
  if (isAttachment(bodyStructure)) {
    results.push({
      filename: bodyStructure.disposition.parameters.filename ||
