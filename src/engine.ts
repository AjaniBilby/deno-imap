...
type GetIncludeResult<T extends ImapMessageInclude> = {
  [K in keyof T as T[K] extends true ? K : never]: K extends keyof ImapMessage ? ImapMessage[K]
    : never;
};

export type FindManyResult<T extends FindManyImapMessageArgs> = T['include'] extends
  ImapMessageInclude ? GetIncludeResult<T['include']>[]
  : ImapMessage[];
...
function RangeId(where: WhereValue<number>) {
  ...
  const min = where.gte ? String(where.gte) : where.gt ? String(where.gt + 1) : '1';

  const max = where.lte ? String(where.lte) : where.lt ? String(where.lt + 1) : '*';
...
export function MatchesWhere(where: ImapMessageWhere, mail: ImapMessage) {
  ...
  if (where.flags) {
    ...
    if (where.flags.hasEvery) {
      for (const f of where.flags.hasEvery) {
        if (!mail.flags.has(f)) return false;
      }
    }

    if (where.flags.hasNone) {
      for (const f of where.flags.hasNone) {
        if (mail.flags.has(f)) return false;
      }
    }
  ...
  if (where.envelope) {
    ...
    if (!MatchWhereScalar(where.envelope.sender, mail.envelope.sender, AddressComparator)) {
      return false;
    }
    if (!MatchWhereScalar(where.envelope.replyTo, mail.envelope.replyTo, AddressComparator)) {
      return false;
    }
...
function MatchWhereValue<T extends string | number | Date>(
  where: WhereValue<T> | undefined,
  val?: T,
): boolean {
...
function MatchWhereScalar<T, X>(
  where: WhereScalar<T> | undefined,
  value: X[],
  comparator: (v: X, r: T) => boolean,
) {
  if (where === undefined) return true;
  ...
  if (where.hasEvery) {
    for (const r of where.hasEvery) {
      if (!value.some((x) => comparator(x, r))) return false;
    }
  }

  if (where.hasNone) {
    for (const r of where.hasNone) {
      if (value.some((x) => comparator(x, r))) return false;
    }
  }
...
export function MakeOrderBy(orderBy?: ImapMessageOrderBy): {
  ...
  const sequenceBy = fetchOrder === 'asc'
